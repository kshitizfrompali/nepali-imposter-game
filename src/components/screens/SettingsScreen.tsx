    ...
    <div className="flex flex-col flex-1 min-h-0">
      <div className="flex-1 overflow-y-auto px-4 pt-2 pb-4 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-black text-white flex-1">{t.settings}</h2>
          <button
            onClick={() => setShowInfo(true)}
            aria-label={t.howToPlay}
            className="min-h-[44px] px-4 rounded-full bg-white/10 hover:bg-white/20 text-white/80 hover:text-white text-xs font-semibold tracking-wide border border-white/10 active:scale-95 transition-all flex items-center gap-1.5 flex-shrink-0"
          >
        ...
        {/* Word mode (classic only — Word Wolf forces random) */}
        {settings.gameMode === 'classic' && (
          <div className="bg-white/5 rounded-2xl p-4 border border-white/10 space-y-3">
            <p className="text-xs font-semibold text-white/50 uppercase tracking-widest">
              {t.wordMode}
